[Skip to main content](https://ytdl-sub.readthedocs.io/en/latest/config_reference/scripting/index.html#main-content)
...
- [.rst](https://ytdl-sub.readthedocs.io/en/latest/_sources/config_reference/scripting/index.rst)
- .pdf
...
_format_ the field using its defined script.
...
considered _static_ because it does not depend on anything from an entry.
...
For context, an _entry_ is a video or audio file downloaded from `yt-dlp`. _Entry_
_variables_ are variables that are derived from an entry’s `info.json` file. This file
...
Instead, we can create a custom _override variable_. This is `ytdl-sub`’s method for
...
`ytdl-sub` is able to _sanitize_ any variable, meaning it replaces any problematic
...
`snake_cased_with_no_spaces`. We can use the [replace](https://ytdl-sub.readthedocs.io/en/latest/config_reference/scripting/scripting_functions.html#replace) _scripting function_ to create and use a snake-cased title.
...
The entirety of an entry’s `info.json` file resides in the [Map](https://ytdl-sub.readthedocs.io/en/latest/config_reference/scripting/scripting_types.html#map)
variable [entry\_metadata](https://ytdl-sub.readthedocs.io/en/latest/config_reference/scripting/entry_variables.html#entry-metadata).

Any field can be accessed by using the [map\_get](https://ytdl-sub.readthedocs.io/en/latest/config_reference/scripting/scripting_functions.html#map-get)
function like so:
